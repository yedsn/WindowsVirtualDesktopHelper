...
Use **Lock All** to mark every eligible window across all virtual desktops as protected from cleanup. Select a row's lock icon, or press `Ctrl+L` for the selected row, to change that individual window's cleanup protection. The helper saves each lock as an exact, case-insensitive application-name and window-title rule, so matching windows remain protected after it restarts. These locks only affect **One-click Cleanup**; they do not prevent you from activating, moving, or manually closing a window.

Use **One-click Cleanup** to request normal closes for eligible, unlocked windows across all virtual desktops. Its label includes the current snapshot's cleanup target count and turns red when one or more windows can be cleaned. Clicking the command first shows the complete target window list, including application and window title; only **Confirm Cleanup** sends the close requests. The command never targets windows shown on all desktops or windows with unknown desktop ownership in the **Other windows** group. It sends normal close requests rather than terminating processes, so each target application can display its normal save prompt or decline to close.
