...
namespace WindowsVirtualDesktopHelper {
	public sealed class DesktopLayoutSnapshotForm : Form {
		...
		public DesktopLayoutSnapshotForm() {
			Text = "Desktop Layout Snapshots";
			StartPosition = FormStartPosition.CenterScreen;
			MinimumSize = new Size(780, 460);
			Size = new Size(980, 620);
			ShowInTaskbar = true;

			var header = new Label { Dock = DockStyle.Top, Height = 54, Padding = new Padding(14, 15, 14, 0), Text = "Desktop Layout Snapshots", Font = new Font(Font.FontFamily, 13, FontStyle.Bold) };
			_snapshots = new ListView { Dock = DockStyle.Fill, View = View.Details, FullRowSelect = true, HideSelection = false, MultiSelect = false };
			_snapshots.Columns.Add("Name", 260);
			_snapshots.Columns.Add("Updated", 170);
			_snapshots.Columns.Add("Created", 170);
			_snapshots.Columns.Add("Desktops", 100);
			_snapshots.Columns.Add("Windows", 100);
		...
		public void RefreshSnapshots() {
			try {
				...
				_status.Text = _items.Count == 0 ? "No desktop layout snapshots saved." : _items.Count + " snapshot" + (_items.Count == 1 ? "" : "s") + " saved.";
		...
		public void RestoreMostRecent() {
			if(_items.Count == 0) {
				_status.Text = "No desktop layout snapshots are available to restore.";
				return;
			}
		...
		private void CreateSnapshot() {
			...
			catch(Exception e) { ShowError("Could not capture the current layout.", e); return; }
			if(!ConfirmCapture(snapshot, true)) return;
			try { App.Instance.DesktopLayoutSnapshots.Create(snapshot); RefreshSnapshots(); _status.Text = "Saved \"" + snapshot.Name + "\" with " + snapshot.WindowCount + " windows."; }
			catch(Exception e) { ShowError("Could not save the snapshot.", e); }
		}
		...
		private void UpdateSelected() {
			...
			try { App.Instance.DesktopLayoutSnapshots.Update(snapshot); RefreshSnapshots(); _status.Text = "Updated \"" + snapshot.Name + "\" without moving any windows."; }
			catch(Exception e) { ShowError("Could not update the snapshot.", e); }
		}

		private bool ConfirmCapture(DesktopLayoutSnapshot snapshot, bool isNew) {
			using(var dialog = new Form { Text = isNew ? "New Desktop Layout Snapshot" : "Update Desktop Layout Snapshot", StartPosition = FormStartPosition.CenterParent, Size = new Size(640, 480), MinimizeBox = false, MaximizeBox = false, FormBorderStyle = FormBorderStyle.FixedDialog }) {
				...
				var save = AddButton(buttons, isNew ? "Save Snapshot" : "Update Snapshot", null); save.DialogResult = DialogResult.OK;
				...
				if(string.IsNullOrWhiteSpace(name.Text)) { MessageBox.Show(this, "A snapshot name is required.", "Desktop Layout Snapshots", MessageBoxButtons.OK, MessageBoxIcon.Warning); return false; }
		...
		private void RestoreSelected() {
			...
			catch(Exception e) { ShowError("Could not analyze the snapshot.", e); return; }
		...
		private void RenameSelected() {
			var selected = SelectedSnapshot;
			if(selected == null) return;
			using(var dialog = new Form { Text = "Rename Snapshot", StartPosition = FormStartPosition.CenterParent, Size = new Size(430, 145), FormBorderStyle = FormBorderStyle.FixedDialog, MinimizeBox = false, MaximizeBox = false }) {
				var name = new TextBox { Dock = DockStyle.Top, Text = selected.Name, Margin = new Padding(12) };
				var buttons = new FlowLayoutPanel { Dock = DockStyle.Bottom, Height = 44, Padding = new Padding(12, 8, 12, 8), FlowDirection = FlowDirection.RightToLeft };
				var save = AddButton(buttons, "Rename", null); save.DialogResult = DialogResult.OK;
				var cancel = AddButton(buttons, "Cancel", null); cancel.DialogResult = DialogResult.Cancel;
				...
				catch(Exception e) { ShowError("Could not rename the snapshot.", e); }
			}
		}

		private void DeleteSelected() {
			var selected = SelectedSnapshot;
			if(selected == null || MessageBox.Show(this, "Delete \"" + selected.Name + "\"? This does not affect open applications or desktops.", "Delete Snapshot", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) != DialogResult.Yes) return;
			try { App.Instance.DesktopLayoutSnapshots.Delete(selected.Id); RefreshSnapshots(); }
			catch(Exception e) { ShowError("Could not delete the snapshot.", e); }
		}
